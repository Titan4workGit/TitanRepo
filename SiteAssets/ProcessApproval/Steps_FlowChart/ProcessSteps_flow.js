	...
	{	 
		...
		if(radioValue == 'User')
		{
			...
			/*	var arrSubVisaLetters = AllEmployee.filter(function (filterData) {
				...
				if(arrSubVisaLetters[0].Email == users[j].EntityData.Email){flag=true;}
		...
		if(radioValue == 'Specific')
		{
		    ...
		    for (var j = 0; j < users.length; j++) 
		    {
		     ...
		     /*   var arrSubVisaLetters = AllEmployee.filter(function (filterData) {
		        ...
		        var QueryCheckActive = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Employees')/items?$select=LogonName/Id,LogonName/Title,Email&$expand=LogonName&$filter=Email eq '"+users[j].EntityData.Email+"' and Status eq 'Active'";
				var arrSubVisaLetters = RequestGetdata(QueryCheckActive);
				if(arrSubVisaLetters.Email == users[j].EntityData.Email){flag=true;}
		        if(arrSubVisaLetters.length==0){userArr.push(users[j].DisplayText)}
		        if(arrSubVisaLetters.length>0){
		        	StepApprver.push({'Name' : arrSubVisaLetters[0].Email , 'Id': arrSubVisaLetters[0].LogonName.Id})	 
		        }   
		...
		$('#'+EditStpID).html(StepDiv)
 	 	...
 	 	for (var j = 0; j < User.length; j++) 
			...
			/*	var arrSubVisaLetters = AllEmployee.filter(function (filterData) {
			    ...
			    var QueryCheckActive = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Employees')/items?$select=LogonName/Id,LogonName/Title,Email&$expand=LogonName&$filter=Email eq '"+User[j].EntityData.Email+"' and Status eq 'Active'";
				var arrSubVisaLetters = RequestGetdata(QueryCheckActive);
			    if(arrSubVisaLetters[0].Email == User[j].EntityData.Email){flag=true;}
			    arrUser.push(arrSubVisaLetters[0].LogonName.Id)	    
			}
...
if(ApproerSteps[i].ApproversId != null ){
   ...
   for(j=0;j<Approvers.length;j++){
	...
	if(arrSubApprover.length>0)
	{
		...
		StepDiv +=	 '<div class="flexitem col-md-4 col-sm-6 col-xs-12"><div class="imgsetion"><div class="empoyeeimg" style="float:left;"><img title="' + arrSubApprover[0].LogonName.Title + '" src="' + Authorattachment + '" alt="" data-themekey="#">';
	...
	stepID = ''	    		
  	if(SavedStpApprovers.length > 0)
  	{
	 	...
	 	for(i=0;i<SavedStpApprovers.length;i++)
	 	{
		 	StpUsers =[];	
 	 		...
 	 		if(SavedStpApprovers[i].Type == 'Role Based')
 	 		{
				...
				var arrUser = RequestGetdata(QueryCheckActive);								
			 	...
			 	if(arrUser.length > 0)
			 	{
				 	if(i == 0){		FirstStpApprover.push(arrUser[0].ManagerLoginName.ID);  	 }
				 	StepApprver.push({'Name' : arrUser[0].ManagerLoginName.Title, 'Id': arrUser[0].ManagerLoginName.ID});
				 	StpUsers.push(arrUser[0].ManagerLoginName.ID);			 
			 	}
			 	else
			 	{
		 	...
		 	{
		 		if(i == 0){	FirstStpApprover.push(_spPageContextInfo.userId);}
	 			StepApprver.push({'Name' : _spPageContextInfo.userDisplayName, 'Id': _spPageContextInfo.userId});
	 			StpUsers.push(_spPageContextInfo.userId);		
...
function SetDisplayInitiator(Action)
{
	...
	{	
		...
		if(Action.value == 'Guest User')
		{
			...
			jQuery('#ddlroles').append('<option value="Indirect Manager">Indirect Manager</option>');
