...
$(document).ready(function () {
    ...
    $('#btnMultiCopy').click(function () {
        ...
        if (arrTempFolder.length > 0) {
            alert("Please select only files to copy.");
            return false;
        }*/
    ...
    $("#divShare").click(function () {
        if (currentSectionType == 'Group Documents') {
            if (LibraryType == "Team" || LibraryType == "Linked") {
                $(".InvisibleClass").hide();
            }
            else {
                $(".InvisibleClass").show();
...
function DisplayFileProperty(Action, SiteURL, Library, CurrentPermission) {
    CopySourceURL = SiteURL;
    if (Library != "Department: IT" && $(".headdingLinks").text() != 'Shared with Me' && $(".headdingLinks").text() != 'Shared by Me' && $(".headdingLinks").text() != 'Archive') {
        IsStopInheritancePermissions = Boolean.parse(Action.dataset.inherit);
        Filepath = Action.dataset.fileserverrelativeurl
    }
    ...
    if (Action.name.includes('/Documents/') == true) {
        Action.name = Action.name.replace("/Documents/", "/Shared%20Documents/");
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            SelectedFileServerURL = tempAction = Action.name;
            ...
            if (propertiesServerRelativeUrl == null) {
                ...
                if (Action.name.includes("DocumentManagementSystem") == true || Action.name.includes("DepartmentalDMS") == true) {
                    ...
                    if (Action.name.search(/\bDocuments\b/) >= 0 || Documentname == undefined || Documentname == '') {
                    ...
                    if (Action.name.search(/\bShared%20Documents\b/) >= 0) {
                        tempAction = Action.name.replace('Shared%20Documents', "Shared Documents");
                    }
                    $(".txtCopyLink").val(HostName + "/Forms/AllItems.aspx?id=" + encodeURIComponent(tempAction) + "&parent=" + encodeURIComponent(tempAction.substr(0, Action.name.lastIndexOf("/") + 0)));
                }
            } else {
                if (Action.name.includes("DocumentManagementSystem") == true) {
                    CopyLibrary = "DocumentManagementSystem";
            ...
            $("#DownloadDocs").prop("href", window.location.origin + Action.name);
            $("#btnFileFullView").prop("href", propertiesServerRelativeUrl);
            if (FileValue.FileLeafRef.includes(".pdf") == true) {//to check if it's PDF
                if (Action.name.search(/\bShared%20Documents\b/) >= 0) {
                    tempAction = Action.name.replace('Shared%20Documents', "Shared Documents");
                }
                var PDFLINK = DMS_Link + "?id=" + encodeURIComponent(tempAction) + "&parent=" + Action.name.substr(0, tempAction.lastIndexOf("/")) + "&p=true";
            ...
            if (FileValue.FileLeafRef.match(/\.(jpeg|jpg|gif|png)$/) != null) {
                if (CurrentPermission == 'Restricted View') {
                    ...
                    var ImageLINK = DMS_Link + "?id=" + encodeURIComponent(tempAction) + "&parent=" + Action.name.substr(0, tempAction.lastIndexOf("/")) + "&p=true";
                ...
                if (Extension == "csv" || Extension == "wmv" || Extension == "avi" || Extension == "mp3" || Extension == "mp4" || Extension == "webm" || Extension == "wma" || Extension == "one" || Extension == "tif" || Extension == "tiff" || Extension == "pdf" || Extension == "xls") {
                    propertiesServerRelativeUrl = $(".txtCopyLink").val();
                }
                else {
                    if (CurrentPermission == 'Restricted View') {
                        propertiesServerRelativeUrl = $("#btnEditViewFile").prop("href");
                        $(".txtCopyLink").val($("#btnEditViewFile").prop("href"));
                        $("#btnFileFullView").prop("href", $("#btnEditViewFile").prop("href"));
                    }
                }
                //Bind File in Iframe
                $('<iframe>', {
            ...
            if ($('.headdingLinks').text() == "Shared by Me") {
                ...
                $(".btnNotSharedTab").hide();
                getSingleShareInfo(DocumentId, Action.rel);
            }
            //Custom Column
            var tempCustom = [];
            if (customColQ != '') {
        ...
        if (Action.name.search(/\bDocuments\b/) >= 0) {
...
function processSendEmails() {
    var body = '<table>' +
        ...
        '<tr>' +
        '<td><p style="margin:0; padding:0;"><b>File Name: </b></p></td>' +
...
function copyFileMulti() {
    ...
    arrFileFolder.forEach(function (entry, index) {
        if (entry.type == 'file') {
            ...
            CopyMoveFolders(entry); debugger;
            if (IsMoving == true) {
                if (entry.SelectedLibrary == "Shared%20Documents") {
                    entry.SelectedLibrary = "Documents";
                }
    ...
    $.when(getItemsWithQueryUsersDocuments(folderName, array.SiteURL)).done(function (DocumentGroup) {
        ...
        files.forEach(function (entry, index) {
            if (IsFileCheckout(folderName, entry.Name, array.SiteURL, '') != true) {
                copyFoldersFile(array, entry);
            }
        });
        folders.forEach(function (entry, index) {
            $.when(CreateMoveCopyFolder(FolderPath, entry.Name, false)).done(function (MainExamListItemTemp) {//Folder created
                CopyFolderName = FolderPath + entry.Name + "/";
                createAllFolder(FolderPath + entry.Name + '/', folderName + entry.Name + '/', array);
            });
        });
    });
...
function copyFoldersFile(array, entry) {
        ...
        if (entry.type.toLowerCase() == 'file' || entry.type.toLowerCase() == "folder") {
            if (IsFileCheckout(folderName, entry.FileFolderName, entry.SiteURL, '') != true && entry.type == 'file') {
                ...
                if (sharedUsersIdArrayListItemCollection.length > 0 && SharingRespose == true) {
                    ...
                    if (txtDocshareingTypePermissionLevel == "Organization") {
                        AddSharedLinkToListOrg(serverRelativeFileUrl, txtSharedDocumetnOnDemandId, entry.type, $('#sharewith').val(), $('#sharedWithPermission').val(), Shared[0], entry.DocType, Shared[1]);
                    }
                    else {
                    ...
                    if ($('#notifymail').prop('checked') == true && PermissionStatus == "")    //If SendMail Checkbox check for file
                    {
                        ...
                        if (SharingUserEmail.length > 0) {
                            ...
                            EmailDesign = EmailDesign + "<div><strong>File Name:</strong> " + $("#FileName").text() + "</div>" +
                                                        "<div><strong>Title:</strong> " + entry.FileTitle + "</div>" +
                                                        "<div><strong>Type:</strong> " + entry.FileType + "</div>" +
                                                        "<div><strong>Reference:</strong> " + entry.FileRef + "</div>" +
...
function shareFile() {
    ...
    if (sharedUsersIdArrayListItemCollection.length > 0 && SharingRespose == true) {
        ...
        Shared = getSharedType();
        if (txtDocshareingTypePermissionLevel == "Organization") {
            AddSharedLinkToListOrg(serverRelativeFileUrl, txtSharedDocumetnOnDemandId, "File", $('#sharewith').val(), $('#sharedWithPermission').val(), Shared[0], $('#FileDocType').text(), Shared[1]);
        }
        else {
        ...
        if ($('#notifymail').prop('checked') == true && PermissionStatus == "")    //If SendMail Checkbox check for file
        {
            ...
            if (SharingUserEmail.length > 0) {
                ...
                EmailDesign = EmailDesign + "<div><strong>File Name:</strong> " + $("#FileName").text() + "</div>" +
                                        ...
                                        "<div><strong>Type:</strong> " + $("#FileDocType").text() + "</div>" +
                                        "<div><strong>Reference:</strong> " + $("#FileReference").text() + "</div>" +
                                         "<div><strong>Permission:</strong> " + Permission + "</div>" +
                                         "<div><strong>Shared By:</strong> " + _spPageContextInfo.userDisplayName + "</div>" +
                                         "<div><strong>Shared On:</strong> " + moment(new Date()).format('MMM DD YYYY') + " " + new Date().toLocaleTimeString().replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, "$1$3") + "</div>" +
                                         ValidityDateHTML +
...
function AddSharedLinkToListOrg(documentURL, documentId, documentType, sharedGroup, permissionType, Shared_Type, DocType, SharedFrom) {
    IsBlock = "No";
    var ListName = "SharedDocument";
    var ItemType = GetItemTypeForListName(ListName);
    var SelectedLibrary = '';
...
function AckUserStatus(SharedItemId, SharedUserName, ServerURL, LibraryURL, Permission, LibraryName, SharedFrom) {
    ...
    $.when(getItemsWithQuery("DocumentAcknowledgement", restQuery, _spPageContextInfo.webAbsoluteUrl)).done(function (DocumentAcknowledgement) {
        if (DocumentAcknowledgement.length > 0) {
            ...
            AckStatus = '<a href="javascript:void(0);" rel="' + SharedItemId + '" name="' + ServerURL + '" onclick="DisplayFileProperty(this, \'' + LibraryURL + '\', \'' + LibraryName + '\', \'' + Permission + '\');" class="AckStatus">Acknowledge</a>';
        }
    });
    return AckStatus;
}
...
function NotifyAllAck() {
    ...
    for (var ack = 0; ack < arrAckAllUser.length; ack++) {
        ...
        EmailDesign = EmailDesign + "<div><strong>File Name:</strong> " + $("#filenamee").text() + "</div>" +
        	                        "<div><strong>Title:</strong> " + $("#filetitle").text() + "</div>" +
        	                        "<div><strong>Type:</strong> " + arrAckAllUser[ack].AckDocType + "</div>" +
...
function EditAddedStep(EmpEmail, DisplayNames, EmpIds) {
    ...
    if ($("#userselection").prop('checked')) {
        ...
        ApprovalStep += '<div class="dropdown"><button class="dropdown-toggle" type="button" data-toggle="dropdown"><img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/AdvanceDocumentSearch/assets/images/dotting.png" alt="">';
...
function AddMoreStep(EmpEmail, DisplayNames, EmpIds) {
    ...
    if ($("#userselection").prop('checked')) {
        ...
        ApprovalStep += '<div style="display:none;"><span id="EmailStep' + StepCount + '">' + EmpEmail.toString() + '</span><span id="EmpIdStep' + StepCount + '">' + EmpIds.toString() + '</span><span id="NameStep' + StepCount + '">' + DisplayNames.toString() + '</span><span id="StepName' + StepCount + '">' + StepName + '</span><span id="TypeStep' + StepCount + '">' + TypeValue + '</span></div>';
        ApprovalStep += '<div class="dropdown ForCustomizeOnly"><button class="dropdown-toggle" type="button" data-toggle="dropdown"><img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/AdvanceDocumentSearch/assets/images/dotting.png" alt="">';
        ApprovalStep += '</button><ul class="dropdown-menu pull-right"><li onclick="EditStep(' + StepCount + ');"><a href="javascript:void(0);" data-toggle="modal" data-target="#addsuccessor"><i class="fa fa-edit" aria-hidden="true"></i> Edit</a></li>';
        ApprovalStep += '<li class="DeleteStep" id="btnDeleteStep' + StepCount + '" onclick="DeleteSeletedStep(' + StepCount + ');"><a href="javascript:void(0);"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</a></li></ul></div>';
        ApprovalStep += '<div class="approvelheadeing"><h3 class="mainheading" id="txtStep' + StepCount + 'Name">' + StepName + '</h3></div><div class="row">';
...
function GetDocumentsSharedWithMe(SectionName) {
    ...
    if (SectionName == 'SharedByMe') {
        var SharedMeColNames = ['File', 'Title', 'Reference', 'Type', 'Shared From', 'Permission', ''];
    }
    else {
        var SharedMeColNames = ['File', 'Title', 'Reference', 'Type', 'Shared By', 'Shared From', 'Permission', ''];
    }
    for (var i = 0; i < SharedMeColNames.length; i++) {
        ColumnName += '<th>' + SharedMeColNames[i] + '</th>';
    }
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (DocumentType.toLowerCase() != "folder") {
            ...
            if (items[i].PermissionStatus != "Revoked" && items[i].PermissionStatus != "Deleted") {
                ...
                documentLink = '<a href="javascript:void(0);" name="' + items[i].DocumentURL + '" rel="' + items[i].Id + '" onclick="DisplayFileProperty(this, \'' + SiteURL + '\', \'' + DMS_Type + '\', \'' + PermissionStatus + '\');">' + Title + '</a>';
        ...
        if ($("#ddlAckFilter").val() == "Acknowledged") {
            if (AckValue.indexOf("Acknowledged") !== -1) {
                ...
                sharedWithMeTR += '<img width="30px" src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/DMS/assets/images/' + Icon + '" alt="' + Icon + '"></label></div></td>';
    ...
    for (var i = 0; i < array.length; i++) {
        ...
        if (jQuery.inArray(array[i].DocumentID, arrDuplicateFile) == '-1') {
            ...
            if (DocumentType.toLowerCase() != "folder") {
                ...
                documentLink = '<a href="javascript:void(0);" name="' + items[0].DocumentURL + '" rel="' + items[0].Id + '" onclick="DisplayFileProperty(this, \'' + SiteURL + '\', \'' + DMS_Type + '\', \'' + PermissionStatus + '\');">' + Title + '</a>';
            ...
            if (PermissionStatus != "All revoked") {
                ...
                if (IsEveryone.length > 0 && IsEveryone.length == temparray.length) {
                    ActionHTML = '<img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/DMS/assets/images/globe.png" style="width:20px; margin:0 2px;" onclick="GetSharedHistory(\'' + items[0].DocumentID + '\', \'' + items[0].DocumentURL + '\', \'' + Title + '\', \'' + DocumentNo + '\', \'' + SourceDocTitle + '\', \'' + DocumentType + '\', \'' + HistoryAction + '\', \'' + array[i].Id + '\')">';
                }
                else {
                    if (IsEveryone.length > 0 && IsEveryone.length != items.length) {
                        ActionHTML = '<img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/DMS/assets/images/shared.png" style="width:20px; margin:0 2px;" onclick="GetSharedHistory(\'' + items[0].DocumentID + '\', \'' + items[0].DocumentURL + '\', \'' + Title + '\', \'' + DocumentNo + '\', \'' + SourceDocTitle + '\', \'' + DocumentType + '\', \'' + HistoryAction + '\', \'' + array[i].Id + '\')"><img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/DMS/assets/images/globe.png" style="width:20px; margin:0 2px;" onclick="GetSharedHistory(\'' + items[0].DocumentID + '\', \'' + items[0].DocumentURL + '\', \'' + Title + '\', \'' + DocumentNo + '\', \'' + SourceDocTitle + '\', \'' + DocumentType + '\', \'' + HistoryAction + '\', \'' + items[0].Id + '\')">';
                    }
                    else {
                        ActionHTML = '<img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/DMS/assets/images/shared.png" style="width:20px; margin:0 2px;" onclick="GetSharedHistory(\'' + items[0].DocumentID + '\', \'' + items[0].DocumentURL + '\', \'' + Title + '\', \'' + DocumentNo + '\', \'' + SourceDocTitle + '\', \'' + DocumentType + '\', \'' + HistoryAction + '\', \'' + array[i].Id + '\')">';
                    }
                }
            }
            else {
            ...
            sharedWithMeTR += '<td class="text-left"><div class="dms-table-ellipsis-2">' + sharedFrom + '</div></td>';
            if (PermissionStatus == "All revoked") {
                sharedWithMeTR += '<td class="text-left"><div class="dms-table-ellipsis-2"><img src="https://cdn.jsdelivr.net/gh/Titan4workGit/TitanRepo@tree/main/SiteAssets/MyDocuments/DMS/assets/images/no_shared.png" style="width:20px; margin:0 2px;"></div></td>';
            }
            else {
        ...
        success: function (data) {
            ...
            if (files.length == 0 && folders.length == 0) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    documentLink = '<a href="javascript:void(0);" name="' + files[i].ServerRelativeUrl + '" rel="" onclick="DisplayFileProperty(this, \'' + SiteURL + '\', \'' + LibraryType + '\', \'' + PermissionType + '\');">' + files[i].Name + '</a>';
