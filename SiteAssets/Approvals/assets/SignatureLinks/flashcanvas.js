...
var BASE_URL = (function(){debugger;
 ...
 * @constructor
 */
function Lookup(array) {debugger;
 ...
 * @constructor
 */
var CanvasRenderingContext2D = function(canvas, swf) {debugger;
    ...
    var self = this
    this._executeCommandIntervalID = setInterval(function() {debugger;
        if (elementIsOrphan(self.canvas)) {debugger;
            clearInterval(self._executeCommandIntervalID)
        } else {
            if (lock[self._canvasId] === 0) {debugger;
...
CanvasRenderingContext2D.prototype = {debugger;
    /*
     * state
     */
...
FlashCanvas[initWindow] = function(window){
    ...
    document.createStyleSheet().cssText =
        CANVAS + "{display:inline-block;overflow:hidden;width:100%;height:300px !important;}";
...
FlashCanvas[unlock] = function(canvasId, url, error) {debugger;
