...
function addGuestUser() {
    ...
    Metadata = {
        __metadata: {
            'type': 'SP.Data.ExternalUsersListItem'
        },
        Title: Email,
...
function editGuestUser(ItemId) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(status=='Active')
            {
                if(propertyAdmin==true){
	                $('#btnViewProperties').show();
	                GetAllTeamMemberDetailsByProjectId(results[0].LoginName.Id);
	                BindProjectGrid(results[0].LoginName.Id);
	            }    
...
function UpdateItem() {
    ...
    $.when(UpdateItemToList(Metadata)).done(function(responseIdmore) {
        ...
        check = false;
        $('#btnViewProperties').hide();
        var fileUpdate = $("#guestUserPick").val();
        if(fileUpdate.length > 0) {
            AddAttachmentsUser(dataID)
        }
        getUser();
        arrPropertiesId.forEach(v => {
            $.when(AssignPermissiononDMSLibary(v.PropertiesURl)).done(function (ProcessListResult) {
                addGuestAsADefaultMemeber(v.Id,v.investorsCount);       
            ...
            for(var i = 0; i < items.length; i++) {
                ...
                if(items[i].InternalMembersId != null) {
                    for(var j = 0; j < Stakeholders.results.length; j++) {
                        ...
                        memberArrs += "_" + stakeholderId + "_" + stakeholderNmae
                    }
                }
                var Supervisor = items[i].Supervisor.Title;
                ...
                dtl = itemId + "_" + Title + "_" + SupervisorId + "_" + Supervisor + "_" + TemplateType + "_" + CustType + "_" + IntSupervisor + "_" + IntSupvId + "_" + memberArrs
...
function AddClientName() {
    ...
    Metadata = {
        __metadata: {
            'type': 'SP.Data.ClientMasterListItem'
        },
        Title: ClientName,
        Client_Code: Code,
        Attn_Name: AttnName,
...
function getProperties() {
    ...
    {
        query=_spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('" + ListName + "')/items?$select=Author/EMail,ProjectSiteURL,TeamMemberCount,InvestorsCount,Address,Modified,MultipleStackeholderId,MultipleSponsorId,MultipleStackeholder/Title,MultipleStackeholder/EMail,MultipleSponsor/Title,MultipleSponsor/EMail,ManagerName/Title,ManagerName/EMail,Id,ProjectName,ProjectCode&$expand=Author,ManagerName,MultipleStackeholder,MultipleSponsor&$filter=CompanyId eq "+Logged_CompanyId+"&$top=5000&$orderby= Modified desc";  //and ManagerNameId eq "+_spPageContextInfo.userId+" 
    }
    else{
        query=_spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('" + ListName + "')/items?$select=Author/EMail,ProjectSiteURL,TeamMemberCount,InvestorsCount,Address,Modified,MultipleStackeholderId,MultipleSponsorId,MultipleStackeholder/Title,MultipleStackeholder/EMail,MultipleSponsor/Title,MultipleSponsor/EMail,ManagerName/Title,ManagerName/EMail,Id,ProjectName,ProjectCode&$expand=Author,ManagerName,MultipleStackeholder,MultipleSponsor&$filter=CompanyId eq "+Logged_CompanyId+" and ManagerNameId eq "+_spPageContextInfo.userId+"&$top=5000&$orderby= Modified desc";
    }
      try {
          $.ajax({
              url: query,
              ...
              success: function (data) {         
                  var resultes=data.d.results;
                  projectListItemArr=resultes;
                  ...
                  {
                      var tblBody='<tr><td style="text-align:center">No record Found!</td></tr>';
                      $('#mainDivAreaOffice365').append(tblBody);
  ...
  var BindProjectGrid =(UserId) =>
{
    if(projectListItemArr.length>0)
    {
        $('#mainDivAreaOffice365').empty();
        $('#exceltable').empty();
        var tblBody='';
        var excelTblBody='';
        projectListItemArr.forEach(element => {
            var propertiesName=element.ProjectName;
            var refNo=element.ProjectCode;
            if(refNo==null)refNo='';
            var Address=element.Address;
            if(Address==null)Address='';
            var Coordinator=element.ManagerName.Title;
            var ProjectSiteURL=element.ProjectSiteURL;
            var teamMembersCount=element.TeamMemberCount;
            if(teamMembersCount==null)teamMembersCount=0;
            var  projectUrlAndId= ProjectSiteURL + "," +element.Id + ","+ teamMembersCount;
            var Investors='';
            if(arrayUserId.length>0)
            {
                Investors=arrayUserId.filter(value => value.ProjectId == element.Id);
            }
            if(Investors.length>0){
                tblBody+='<tr><td><input type="checkbox" value="'+projectUrlAndId+'" checked="checked" disabled="disabled" class="checked"></td>';
            }
            else
            {
                tblBody+='<tr><td><input type="checkbox" value="'+projectUrlAndId+'" class="checkPro"></td>';
            }
            tblBody+='<td><div class="property_ns"><h3>'+propertiesName+'</h3></div></td><td><div class="property_coord"><h3>'+Coordinator+'</h3></div></td>'
            if(Investors.length>0 ){
                 tblBody+='<td><a class="custom-edit-btn remove-user" onclick="DeleteMembers('+Investors[0].Id+','+UserId+')" href="#"><i class="fa fa-trash-o"></i></a></td>';
            }
            else
            {
               tblBody+='<td></td>';
            ...
            tblBody+='</tr>';
            //excelTblBody+='<tr><td>'+propertiesName+'</td><td>'+refNo+'</td><td>'+Address+'</td><td>'+Coordinator+'</td><td>'+Investors+'</td><td>'+Investors+'</td></tr>';
            
            
        });
        $('#mainDivAreaOffice365').append(tblBody);
        $('#exceltable').append(excelTblBody);
        //GenerateTableMyProjectList();
    }
    else
    {
        tblBody+='<tr><td style="text-align:center">NorecordFound</td></tr>';
        $('#mainDivAreaOffice365').append(tblBody);
        //GenerateTableMyProjectList()
    }            
...
var arrPropertiesId=[];
var splitCheckedValue =() =>{
    arrPropertiesId=[];
    if(g_Status==false)
    {
        alert('unauthorized access');
        $('#mainDivAreaOffice365').empty();
        $('#properties_lsting').modal('hide');

        return false;
    }
    $.each($("input[class='checkPro']:checked"), function(){
        var checkedValue=$(this).val();
        checkedValue=checkedValue.split(',');
        arrPropertiesId.push({
            Id:checkedValue[1],
            PropertiesURl:checkedValue[0],
            investorsCount:checkedValue[2]
        });
    });
}

/////// Submit Button Operations for Adding Manager as Default Team Member (Add/Update)
function addGuestAsADefaultMemeber(projectID,userCount) {
    var dfd = $.Deferred();
    var ListName = 'ProjectTeamDetails';
    var metadataDescription = CreateMetadataForAddingProjectManager(ListName,projectID);
    ...
    $.ajax({
        ...
        success: function (data) {
            //dfd.resolve(data);
            console.log("Guest user Added As A Default Team Member.");
            updateItemsInPrppertiesList(projectID,userCount)
...
function updateItemsInPrppertiesList(txtProjectID,count) 
{
	var dfd = $.Deferred();
    var ListName='ProjectDetails';
    var Metadata='';
    var ItemType = GetItemTypeForListName(ListName);
    var type=$('#viewModalHeaders').text();
    var investorsId=[];
    investorsId.push(guestLoginId);
    Metadata = {
        __metadata: {
                'type': ItemType
        },
        InvestorsCount:parseInt(count)+1,
        MultipleSponsorId: { 'results': investorsId},
    }
	$.ajax({
    ...
    {
        console.log('Counter add successfully ')
        getProperties();
	},
    error: function (error) 
    {
...
function CreateMetadataForAddingProjectManager(ListName, projectID) {
    var itemType = GetItemTypeForListName(ListName);
    var Metadata;
    Metadata = { "__metadata": { 'type': itemType } };
    Metadata["ProjectId"] = projectID;
    Metadata["TeamMemberId"] = guestLoginId;
    Metadata["Role"] ='Guest';
    Metadata["Usertype"] ='Investors';    
    Metadata["CompanyId"] = Logged_CompanyId;
    return Metadata;
}

function AssignPermissiononDMSLibary(dmsSiteURL) {
    var permissionLevel='1073741826'
    var headers = {
        "Accept": "application/json;odata=verbose",
        "content-Type": "application/json;odata=verbose",
        "X-RequestDigest": jQuery("#__REQUESTDIGEST").val()
    }
    var webUrl = dmsSiteURL + "/_api/web/lists/getByTitle('Documents')/roleassignments/addroleassignment(principalid=" + guestLoginId + ",roleDefId=" + permissionLevel + ")";
    $.ajax(
    {
        url: webUrl,
        type: "POST",
...
var arrayUserId=[];
function GetAllTeamMemberDetailsByProjectId(userID) {
    var Ownurl = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('ProjectTeamDetails')/items?$select=Id,ProjectId&$Filter=TeamMemberId eq " + userID + " and CompanyId eq '"+Logged_CompanyId+"'&$top=5000&$orderby= Modified desc";
    $.ajax({
        url: Ownurl,
        ...
        success: function (data) {
            //var items=data.d.results;
            arrayUserId=[];
            arrayUserId=data.d.results;          

        }    
    })
}

var DeleteMembers =(itemsId,userId) =>{
    var url=_spPageContextInfo.webAbsoluteUrl+ "/_api/web/lists/getByTitle('ProjectTeamDetails')/items("+itemsId+")";
        ...
        try {
            $.ajax({
                ...
                success:function(v){
                    ...
                    GetAllTeamMemberDetailsByProjectId(userId);
                    BindProjectGrid(userId)        
                },
                error:function(v){
                    console.log(v.error);
                }
...
var propertyAdmin=false;
function propertyPermission() {
    var deffered = $.Deferred();
    var result;
    var listName = 'EnvironmentalSettings';
    var siteURL = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('" + listName + "')/items?$select=Id&$filter=Title eq 'Property' and Active eq 1 ";
    $.ajax({
        url: siteURL,
        headers: { Accept: "application/json;odata=verbose" },
        async: false,
